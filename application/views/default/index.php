...
<div class='posts-list row'>
  <?php foreach ($artikel as $post) {?>
    <div class="list-post col-sm-4">
      <div class='dalam'>
        <div class="gambar">
          <a href="<?php echo base_url('post/index/'.$post['tahun'].'/'.intToStr($post['bulan']).'/'.$post['url']); ?>"><img src="<?php echo base_url('uploads/'.$post['thumbnailWidth'].'x'.$post['thumbnailHeight'].'/'.$post['thumbnail']); ?>"/></a>
        </div>
        <div class="meta-post">
          <div class="stuff_article_inner">
            ...
            <h2><a href="<?php echo base_url('post/index/'.$post['tahun'].'/'.intToStr($post['bulan']).'/'.$post['url']); ?>"><?php echo strip_quotes(strip_tags($post['title'])); ?></a></h2>
